...
class Sword extends ControllableDirection implements Entity{
  ...
  Sword(Entity wielder){ this.wielder=wielder; } //constructor 
  public Point location(){ //location of sword 
  ...
  public void onHit(Model m, Entity e){
    if(e instanceof Monster){ m.remove(e); } //if sword hit monster 
  }

  public double effectRange(){ return 0.3d; } 
  
  public void ping(Model m){ //using of the sword 
    ...
    m.entities().stream()
      ...
      .forEach(e->onHit(m,e));
  } //go through all locations around the sword and filter for those that hit and those that did not have effect 
  public void draw(Graphics g, Point center, Dimension size) {
    drawImg(Img.Sword.image, g, center, size); //draws the sword 
  }
}