...
class Compact extends JFrame{ 
  ...
  Runnable closePhase = ()->{}; //Its a Runnable with an empty run definition. Anonymous class representation.
  Phase currentPhase;
  Compact(){ //class 
    assert SwingUtilities.isEventDispatchThread();
    setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
    phaseZero(); //starts game
    setVisible(true);
  ...
  private void phaseZero() {
    ...
    closePhase.run();
    closePhase=()->{ //removes text from GUI
     remove(welcome);
     remove(start);
     };
  ...
  private void phaseOne(){ //two phases to switch between from one move up to the next 
